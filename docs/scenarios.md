...
| `fork-cascade.yaml` | A single chain forks, and one of those forks forks again, building a tree three generations deep — six permanently-live branches, three of them forks off of forks, each kept alive by a node group that never switches rulesets again once it reaches its own leaf. |
